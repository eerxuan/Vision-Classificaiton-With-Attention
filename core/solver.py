...
class CaptioningSolver(object):
    ...
    def train(self):
        ...
        )  # set the 'reuse' of each variable as True
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            tf.summary.scalar('batch_loss',
                              loss)  # add those to the observing process
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                for i in range(n_iters_per_epoch):
                    ...
                    label_batch_idxs = np.array(
                        [[self.model.label_to_idx[per] for per in PER]
                         for PER in labels_batch])

                    feed_dict = {
                        self.model.features: features_batch,
                        self.model.label_idxs: label_batch_idxs
                    }
                    ...
                    if (i + 1) % self.print_every == 0:
                        ...
                        gen_label_idxs, gen_labels = decode(
                            sam_labels_list, self.model.idx_to_label)
                        ...
                        sam_labels_list = np.transpose(sam_labels_list)
                        if acc_per_frame:
                            AP = accurate_percentage_frame(
                                sam_labels_list, label_batch_idxs)
                        else:
                            AP = accurate_percentage(gen_label_idxs,
                                                     org_label_idxs)
                        print('the current accuracy rate: ' + str(AP))
                ...
                if (e + 1) % self.save_every == 0:
                    saver.save(
                        sess, self.model_path + 'model', global_step=e + 1)
                    print("model-%s saved." % (e + 1))

    # test one model in test set
    def test(self):
        # train dataset
        with open('test_data_vgg.pkl', 'rb') as handle:
        ...
        # features = np.array(test_data['features'])
        # labels = np.array(test_data['labels'])
        video_ids = np.array(test_data['new_filename'])
        ...
        percentage_txt = open(test_result_save_path + 'percentage record.txt',
                              'a')
        ...
        with tf.Session(config=config) as sess:
            ...
            for iter in range(n_iterations):
                ...
                feed_dict = {
                    self.model.features: features_batch,
                    self.model.label_idxs: label_idxs
                }
                alps, bts, sam_label_list_test = sess.run(
                    [alphas, betas, sam_labels_test], feed_dict)

                # decode the obtained result
                gen_idxs, gen_labels = decode(sam_label_list_test,
                                              self.model.idx_to_label)
                ...
                for i in range(len(labels_batch)):
                    txt_file.write(
                        str(video_id_batch[i]) + '-- org_label: ' +
                        str(org_idxs[i]) + '--V.S.-- gen_label: ' +
                        str(gen_idxs[i]) + '\n')

                    # save the match result
                    MATCH_RESULT[video_id_batch[i]] = gen_idxs[i] - org_idxs[i]

                sam_label_list_test = np.transpose(sam_label_list_test)
                if acc_per_frame:
                    AP = accurate_percentage_frame(sam_label_list_test,
                                                   label_idxs)
                else:
                ...
                percentage_txt.write(
                    str(iter) + ' batch -- accurate percentage: ' + str(AP) +
                    '\r\n')

                with open(test_result_save_path + 'alpha_test_%d.pkl' % iter,
                          'wb') as f:
                    pickle.dump(alps, f)

                with open(
                        test_result_save_path + 'video_id_test_%d.pkl' % iter,
                        'wb') as f:
                    pickle.dump(video_id_batch, f)

                with open(
                        test_result_save_path + 'gen_idxs_test_%d.pkl' % iter,
                        'wb') as f:
                    pickle.dump(sam_label_list_test, f)

                # print(np.array(sam_label_list_test).shape)
                # print(np.array(label_idxs).shape)
                # print(np.array(gen_idxs).shape)
                # print(np.array(org_idxs).shape)
            ...
            print('The total accurate percentage is ' + str(np.mean(AP_ALL)) +
                  '\r\n')
            hickle.dump(MATCH_RESULT,
                        test_result_save_path + 'MATCH_RESULT.hkl')
    ...
    def all_model_val(self):
        ...
        models = [
            per for per in list(os.walk(models_path))[0][-1]
            if per[-5:] == 'index'
        ]
        models = sorted(models, key=self.cmp_to_key(model_comp))
        ...
        with tf.Session(config=config) as sess:
            ...
            for cur_model in models:
                saver.restore(sess, self.model_path + cur_model[:-6])
                AP_ALL = 0
                for iter in range(n_iterations):
                    S = iter * self.batch_size
                    E = (iter + 1) * self.batch_size
                    ...
                    AP_ALL += float(AP)
                print(cur_model + ': ' +
                      str(round(AP_ALL / float(n_iterations), 4) * 100) + '%')
