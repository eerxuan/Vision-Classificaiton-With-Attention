...
class CaptionGenerator(object):
    def __init__(self,
        ...
        comp = lambda x, y: 1 if label_to_idx[x] > label_to_idx[
            y] else -1 if label_to_idx[x] < label_to_idx[y] else 0
        self.idx_to_label = sorted(
            label_to_idx.keys(), key=self.cmp_to_key(comp))
        ...
        self.features = tf.placeholder(
            tf.float32, [None, self.n_time_step, self.L, self.D])
    ...
    def _get_initial_lstm(self, features, name):
        with tf.variable_scope(name):
            ...
            b_h = tf.get_variable(
                'b_h', shape=[self.H], initializer=self.const_initializer)
            ...
            w_c = tf.get_variable(
                ...
                initializer=self.weight_initializer)
            b_c = tf.get_variable(
                'b_c', shape=[self.H], initializer=self.const_initializer)
    ...
    def _attention_layer(self, features, features_proj, h, reuse):
        with tf.variable_scope('attention_layer', reuse=reuse):
            w = tf.get_variable(
                ...
                initializer=self.weight_initializer)
            b = tf.get_variable(
                'b', shape=[self.D], initializer=self.const_initializer)
            w_att = tf.get_variable(
                'w_att',
                shape=[self.D, 1],
                initializer=self.weight_initializer)

            h_att = tf.nn.relu(features_proj +
                               tf.expand_dims(tf.matmul(h, w), 1) + b)
            out_att = tf.reshape(
                tf.matmul(tf.reshape(h_att, [-1, self.D]), w_att),
                [-1, self.L])
    ...
    def _selector(self, context, h, reuse):
        with tf.variable_scope('selector', reuse=reuse):
            w = tf.get_variable(
                'w', shape=[self.H, 1], initializer=self.weight_initializer)
            b = tf.get_variable(
                'b', shape=[1], initializer=self.const_initializer)
    ...
    def _decode_lstm(self, h, context, dropout, reuse):
        with tf.variable_scope('logits', reuse=reuse):
            w_h = tf.get_variable(
                ...
                initializer=self.weight_initializer)
            b_h = tf.get_variable(
                'b_h', shape=[self.V], initializer=self.const_initializer)
            w_out = tf.get_variable(
                'w_out',
                shape=[self.V, self.V],
                initializer=self.weight_initializer)
            b_out = tf.get_variable(
                'b_out', shape=[self.V], initializer=self.const_initializer)
    ...
    def build_model(self):
        ...
        features = self._batch_norm(
            features, mode='test&val', name='Conv_features')
        ...
        for t in range(self.T):  # each t of lstm-layers process
            ...
            logits = self._decode_lstm(
                h, context, dropout=self.dropout, reuse=(t != 0))
            loss += tf.reduce_sum(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    logits=logits, labels=self.label_idxs[:, t]))
        # LSTM ----------------------one layer----------------------------- END
