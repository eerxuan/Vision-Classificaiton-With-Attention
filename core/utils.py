...
# calculate the AP
def accurate_percentage_frame(x, y):
    isSame = x - y
    return round(
        float(sum([1 for row in isSame
                   for each in row if each == 0])) / float(len(x) * len(x[0])),
        4)
...
def accurate_percentage(x, y):
    isSame = x - y
    return round(
        float(sum([1 for each in isSame if each == 0])) / float(len(x)), 4)


def model_comp(x, y):
    num1 = int(x[x.find('-') + 1:-6])
    num2 = int(y[y.find('-') + 1:-6])
    if num1 > num2:
        return 1
    if num1 < num2:
        return -1
...
def load_data(data_path, split):
    start_t = time.time()
    features = hickle.load(data_path + split + '/' + 'features_' + split +
                           '.hkl')
    with open(data_path + split + '/' + 'labels_all_' + split + '.pkl',
              'rb') as pickle_file:
        labels = pickle.load(pickle_file)
#     labels = hickle.load(data_path + split + '/' + 'labels_all_' + split + '.hkl')
    video_ids = load_pickle(data_path + split + '/' + 'video_names_' + split +
                            '.pkl')  # name == id
    video_filename = load_pickle(data_path + split + '/' + 'video_filenames_' +
                                 split + '.pkl')
    data = {
        'features': features,
        'labels': labels,
